...
public class OverlayPreferencesTest {
    ...
    @Test
    public void touchKitOptions_AreReadFromProfilePreferences() {
        ProfilesManager pm = ProfilesManager.getInstance();
        pm.load(ctx);

        String json = "{\"checkbox_touchkit_cloud_gaming_mode\":true," +
                "\"checkbox_touchkit_adjustable_overlay\":true," +
                "\"checkbox_touchkit_disable_gestures\":true}";
        Type type = new TypeToken<Map<String, Object>>(){}.getType();
        Map<String, Object> options = new Gson().fromJson(json, type);

        SettingsProfile profile = new SettingsProfile(UUID.randomUUID(), "TouchKit", 0, 0, options);
        pm.add(profile);
        pm.setActive(profile.getUuid());

        PreferenceConfiguration config = PreferenceConfiguration.readPreferences(ctx);
        assertTrue(config.touchkitCloudGamingMode);
        assertTrue(config.touchkitAdjustableOverlay);
        assertTrue(config.touchkitDisableGestures);
    }
    ...
    public void legacyAlwaysShowSwitch_MigratesToUnifiedFloatingControls() {
        ...
        String json = "{\"checkbox_enable_keyboard\":true}";
        Type type = new TypeToken<Map<String, Object>>(){}.getType();
        Map<String, Object> options = new Gson().fromJson(json, type);
        SettingsProfile profile = new SettingsProfile(UUID.randomUUID(), "Legacy TouchKit", 0, 0, options);
        pm.add(profile);
        pm.setActive(profile.getUuid());
