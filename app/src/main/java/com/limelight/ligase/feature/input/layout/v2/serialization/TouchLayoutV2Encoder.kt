...
object TouchLayoutV2Encoder {
    fun encode(document: TouchLayoutV2Document): ByteArray {
        val withoutHash = documentObject(document, "")
        val hash = TouchLayoutV2Jcs.contentHash(withoutHash)
        val complete = StrictJsonValue.ObjectValue(
            withoutHash.fields + ("contentHash" to str(hash)),
        )
        return TouchLayoutV2Jcs.canonicalBytes(complete)
    }
    ...
    private fun recommendation(value: LayoutRecommendation): StrictJsonValue {
        val fields = linkedMapOf<String, StrictJsonValue>(
            ...
            "safeAreaPolicy" to str(
                when (value.safeAreaPolicy) {
                    SafeAreaPolicy.VIDEO_CONTENT -> "videoContent"
                    SafeAreaPolicy.VIDEO_CONTENT_AND_SYSTEM_INSETS ->
                        "videoContentAndSystemInsets"
                },
            ),
        )
    ...
    private fun payload(value: ControlPayload): StrictJsonValue = when (value) {
        ...
        is DirectionalPayload -> {
            ...
            StrictJsonValue.ObjectValue(fields)
        }
        is ChordPayload -> {
            val fields = linkedMapOf<String, StrictJsonValue>(
                "payloadKind" to str(kind(value.kind)),
    ...
    private fun rect(value: IntRect) = obj(
        "x" to int(value.x), "y" to int(value.y),
    ...
    private fun nullable(value: String?) =
        value?.let(::str) ?: StrictJsonValue.NullValue
    private fun nullable(value: Int?) =
        value?.let(::int) ?: StrictJsonValue.NullValue
    private fun obj(vararg values: Pair<String, StrictJsonValue>) =
        StrictJsonValue.ObjectValue(linkedMapOf(*values))
    private fun array(values: List<StrictJsonValue>) = StrictJsonValue.ArrayValue(values)
    private fun str(value: String) = StrictJsonValue.StringValue(value)
    private fun int(value: Int) = StrictJsonValue.IntegerValue(value.toLong())
    private fun int(value: Long) = StrictJsonValue.IntegerValue(value)
    private fun bool(value: Boolean) = StrictJsonValue.BooleanValue(value)
}
