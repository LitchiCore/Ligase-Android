...
object TouchLayoutV2Codec {
    ...
    fun decode(raw: ByteArray): TouchLayoutV2Document = decode(raw, allowEmptyDraft = false)

    internal fun decodeDraft(raw: ByteArray): TouchLayoutV2Document =
        decode(raw, allowEmptyDraft = true)

    private fun decode(raw: ByteArray, allowEmptyDraft: Boolean): TouchLayoutV2Document {
        val root = try {
    ...
    private fun parseVariant(
        ...
        val elements = obj.array("elements", path).bounded(
            if (allowEmptyDraft) 0 else 1,
            512,
            "$path/elements",
        )
