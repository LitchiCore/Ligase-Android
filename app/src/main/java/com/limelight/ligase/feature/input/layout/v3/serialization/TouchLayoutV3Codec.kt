...
object TouchLayoutV3Codec {
    ...
    private fun decode(raw: ByteArray, allowEmptyDraft: Boolean): TouchLayoutV3Document {
        ...
        root.long("schemaVersion", "").requireExact(3L, "unsupportedSchema")
        ...
        TouchLayoutV3Validator.validate(document)
        if (TouchLayoutV3Jcs.contentHash(root) != contentHash) fail("contentHashMismatch")
        return document
    }
    ...
    private fun parseRecommendation(
        obj: StrictJsonV3Value.ObjectValue,
        path: String,
    ): LayoutRecommendation {
        obj.exactKeys(
            setOf(
                "preferredAspectRatio", "minAspectRatio", "maxAspectRatio",
                "minShortestSideDp", "minTouchTargetDp",
    ...
    private fun parseRatio(obj: StrictJsonV3Value.ObjectValue, path: String): AspectRatio {
    ...
    private fun parseSize(obj: StrictJsonV3Value.ObjectValue, path: String): IntSize {
    ...
    private fun parseElement(
        ...
        rectObj.exactKeys(setOf("horizontalOffset", "verticalOffset", "width", "height"), "$path/rect")
        val rect = AnchoredRect(
            rectObj.int("horizontalOffset", "$path/rect").range(-32768, 32767, "$path/rect/horizontalOffset"),
            rectObj.int("verticalOffset", "$path/rect").range(-32768, 32767, "$path/rect/verticalOffset"),
        ...
        return TouchLayoutV3Element(
            ...
            kind = kind,
            rect = rect,
            anchorX = when (obj.string("anchorX", path)) {
                "LEFT" -> HorizontalAnchor.LEFT
                "CENTER" -> HorizontalAnchor.CENTER
                "RIGHT" -> HorizontalAnchor.RIGHT
                else -> fail("schemaRejected", "$path/anchorX")
            },
            anchorY = when (obj.string("anchorY", path)) {
                "TOP" -> VerticalAnchor.TOP
                "BOTTOM" -> VerticalAnchor.BOTTOM
                else -> fail("schemaRejected", "$path/anchorY")
            },
    ...
    private fun parseRadial(obj: StrictJsonV3Value.ObjectValue, path: String): RadialPayload {
        obj.exactKeys(
    ...
    private fun parseScroll(obj: StrictJsonV3Value.ObjectValue, path: String): ScrollPayload {
        obj.exactKeys(
            setOf(
    ...
    private fun parseGyro(obj: StrictJsonV3Value.ObjectValue, path: String): GyroPayload {
        obj.exactKeys(
            setOf(
    ...
    private fun parseTrigger(obj: StrictJsonV3Value.ObjectValue, path: String): Pair<Trigger, Int?> {
    ...
    private fun parseAppearance(obj: StrictJsonV3Value.ObjectValue, path: String): Appearance {
    ...
    private fun parseInputCodeList(
        array: StrictJsonV3Value.ArrayValue,
        path: String,
    ): List<InputCode> = array.bounded(1, 16, path)
        .mapIndexed { index, value -> parseInputCode(value.obj("$path/$index"), "$path/$index") }

    private fun parseInputCode(obj: StrictJsonV3Value.ObjectValue, path: String): InputCode {
    ...
    private fun validateExtensions(
        fields: Map<String, StrictJsonV3Value>,
        path: String,
        depth: Int,
    ) {
    ...
    private fun validateExtensionValue(value: StrictJsonV3Value, path: String, depth: Int) {
        when (value) {
            StrictJsonV3Value.NullValue, is StrictJsonV3Value.BooleanValue -> Unit
            is StrictJsonV3Value.IntegerValue ->
                if (value.value !in -SAFE_MAX..SAFE_MAX) fail("unsafeInteger", path)
            is StrictJsonV3Value.StringValue -> value.value.clean(0, 256, path)
            is StrictJsonV3Value.ArrayValue -> {
            ...
            is StrictJsonV3Value.ObjectValue -> {
                if (depth >= 6) fail("extensionDepthExceeded", path)
    ...
    private fun StrictJsonV3Value.ObjectValue.payloadKind(expected: String, path: String) {
        string("payloadKind", path).requireExact(expected, "payloadKindMismatch")
    }

    private fun StrictJsonV3Value.ObjectValue.exactKeys(
        allowed: Set<String>,
        path: String,
    ...
    private fun StrictJsonV3Value.ObjectValue.value(name: String, path: String): StrictJsonV3Value =
        fields[name] ?: fail("schemaRejected", "$path/$name")
    private fun StrictJsonV3Value.ObjectValue.string(name: String, path: String) = value(name, path).string("$path/$name")
    private fun StrictJsonV3Value.ObjectValue.optionalString(name: String, path: String): String? =
        fields[name]?.string("$path/$name")
    private fun StrictJsonV3Value.ObjectValue.long(name: String, path: String) = value(name, path).long("$path/$name")
    private fun StrictJsonV3Value.ObjectValue.int(name: String, path: String) = value(name, path).int("$path/$name")
    private fun StrictJsonV3Value.ObjectValue.optionalInt(name: String, path: String): Int? =
        when (val value = fields[name]) {
            null, StrictJsonV3Value.NullValue -> null
            else -> value.int("$path/$name")
        }
    private fun StrictJsonV3Value.ObjectValue.boolean(name: String, path: String) =
        (value(name, path) as? StrictJsonV3Value.BooleanValue)?.value ?: fail("schemaRejected", "$path/$name")
    private fun StrictJsonV3Value.ObjectValue.obj(name: String, path: String) = value(name, path).obj("$path/$name")
    private fun StrictJsonV3Value.ObjectValue.array(name: String, path: String) = value(name, path).array("$path/$name")
    private fun StrictJsonV3Value.ObjectValue.optionalArray(name: String, path: String) =
        fields[name]?.array("$path/$name")

    private fun StrictJsonV3Value.obj(path: String) =
        this as? StrictJsonV3Value.ObjectValue ?: fail("schemaRejected", path)
    private fun StrictJsonV3Value.array(path: String) =
        this as? StrictJsonV3Value.ArrayValue ?: fail("schemaRejected", path)
    private fun StrictJsonV3Value.string(path: String) =
        (this as? StrictJsonV3Value.StringValue)?.value ?: fail("schemaRejected", path)
    private fun StrictJsonV3Value.long(path: String) =
        (this as? StrictJsonV3Value.IntegerValue)?.value ?: fail("schemaRejected", path)
    private fun StrictJsonV3Value.int(path: String): Int {
        val value = long(path)
        if (value !in Int.MIN_VALUE..Int.MAX_VALUE) fail("schemaRejected", path)
        return value.toInt()
    }

    private fun StrictJsonV3Value.ArrayValue.bounded(min: Int, max: Int, path: String): List<StrictJsonV3Value> {
        if (values.size !in min..max) fail("schemaRejected", path)
        return values
    }

    private fun String.clean(min: Int, max: Int, path: String): String {
        StrictJsonV3.validateScalarString(this)
