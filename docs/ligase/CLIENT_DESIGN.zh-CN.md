...
设置页可安全展示RTT、抖动、丢包、估算码率与建议码率，但只有经过权威验证的结果才可进入
完成态。建议不是质量保证，且不会自动修改码率、公网、防火墙或路径选择。Host网络质量契约
未就绪时，入口明确显示不可用，不以运行中串流统计或本机探测冒充预检结果。

设备在线心跳只在应用前台维护当前已认证Host，或由正在串流的Activity维护其实际证书绑定Host。
普通后台、onStop、Doze、force-stop和进程结束不会补发最后一次心跳，也不会为“看起来在线”启动
后台服务。客户端收到成功响应只表示本次请求经过严格校验，不自行宣布设备在线；presence仍由
Host Core基于认证证书与单调TTL判定，且与RTSP session状态正交。

分辨率编辑器在手机、较矮窗口、旋转、大字体和IME打开时保持标题与取消/保存操作可达；中间的
预设、禁用原因、自定义宽高与校验错误可按阅读顺序纵向滚动。不得通过删除档位或缩小字体规避
空间限制，系统Back仍是零写取消。
