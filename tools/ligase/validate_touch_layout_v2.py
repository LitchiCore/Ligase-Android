#!/usr/bin/env python3
"""Strict validator and deterministic vectors for Ligase Touch Layout v2."""
...
def jcs_text(value) -> str:
    """RFC 8785 for the v2 value domain (no floats; safe integers only)."""
    if value is None:
        return "null"
    if value is True:
        return "true"
...
def _intersection(left, right):
    ...
    if x2 <= x1 or y2 <= y1:
        fail("incompatibleSafeArea")
    return {"x": x1, "y": y1, "width": x2 - x1, "height": y2 - y1}


def map_element(canvas, element, video_rect, system_safe_rect, policy):
    safe = video_rect if policy == "videoContent" else _intersection(video_rect, system_safe_rect)
    if safe["width"] <= 0 or safe["height"] <= 0:
        fail("incompatibleSafeArea")
    if safe["width"] * canvas["height"] <= safe["height"] * canvas["width"]:
        scale_num, scale_den = safe["width"], canvas["width"]
    else:
        scale_num, scale_den = safe["height"], canvas["height"]
    mapped_canvas_width = _round_half_up(canvas["width"] * scale_num, scale_den)
    mapped_canvas_height = _round_half_up(canvas["height"] * scale_num, scale_den)
    horizontal_residual = safe["width"] - mapped_canvas_width
    vertical_residual = safe["height"] - mapped_canvas_height
    horizontal_factor = {"left": (0, 1), "center": (1, 2), "right": (1, 1)}[
        element["horizontalAnchor"]
    ]
    vertical_factor = {"top": (0, 1), "center": (1, 2), "bottom": (1, 1)}[
        element["verticalAnchor"]
    ]
    offset_x = _round_half_up(horizontal_residual * horizontal_factor[0], horizontal_factor[1])
    offset_y = _round_half_up(vertical_residual * vertical_factor[0], vertical_factor[1])
    rect = element["rect"]
    left = safe["x"] + offset_x + _round_half_up(rect["x"] * scale_num, scale_den)
    top = safe["y"] + offset_y + _round_half_up(rect["y"] * scale_num, scale_den)
    right = safe["x"] + offset_x + _round_half_up(
        (rect["x"] + rect["width"]) * scale_num, scale_den
    )
    bottom = safe["y"] + offset_y + _round_half_up(
        (rect["y"] + rect["height"]) * scale_num, scale_den
    )
...
def validate(document):
    if document.get("format") != "ligase-touch-layout" or document.get("schemaVersion") != 2:
        fail("invalidFormat")
    ...
    for vi, variant in enumerate(variants):
        ...
        for ei, element in enumerate(elements):
            ...
            if (
                ...
                fail("rectOutsideCanvas", path + "/rect")
            ...
            if element["enabled"] and not element["hidden"]:
                active.append(element)
        for index, left in enumerate(active):
            for right in active[index + 1:]:
                if intersects(left["rect"], right["rect"]) and left["zOrder"] == right["zOrder"]:
...
def run_viewport_vectors(path: Path):
    ...
    for case in vectors["viewportCases"]:
        try:
            actual = map_element(
                case["canvas"], case["element"], case["videoContentRect"],
                case["systemSafeRect"], case["safeAreaPolicy"],
            )
